<template>
  <div class="main-container">
    ...
    <md-dialog :md-active.sync="editTogal">
      ...
      <md-dialog-content>
        <Edit v-on:close-dialog="refresh" :privacy="privacy" />
      </md-dialog-content>
    </md-dialog>
    <!-- add dialog -->
    ...
    <md-table md-card>
      <md-table-toolbar>
        <div class="md-toolbar-section-start">
          <h1 class="md-title">Privacies</h1>
        </div>
      </md-table-toolbar>
      ...
      <md-table-row v-for="privacy in privacies" :key="privacy.id">
        <md-table-cell md-numeric>{{ privacy.id }}</md-table-cell>
        <md-table-cell>{{ privacy.title }}</md-table-cell>
        <md-table-cell>{{ privacy.body }}</md-table-cell>

        <md-table-cell md-label="Actions">
          <md-button class="md-icon-button md-primary" @click="edit(privacy)">
            <md-icon>edit</md-icon>
          </md-button>
          <md-button class="md-icon-button md-accent" @click="remove(privacy.id)">
...
export default {
  ...
  methods: {
    ...
    confirm() {
      axios
        .delete("admin/privacy/" + this.selectedId)
        .then((res) => {
