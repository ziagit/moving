    ...
    <md-table md-card>
      <md-table-toolbar>
        <div class="md-toolbar-section-start">
          <h1 class="md-title">Moving Sizes</h1>
        </div>
      </md-table-toolbar>
      ...
      <md-table-row v-for="size in sizes" :key="size.id">
        ...
        <md-table-cell v-if="size.code != 'fewitems'"
          >{{ size.hours_to_move }} hrs</md-table-cell
        >
        <md-table-cell v-else>
          <md-button class="md-icon-button md-primary" to="/admin/moving-few-items">
            <md-icon>add</md-icon>
            <md-tooltip>Add item</md-tooltip>
          </md-button>
        </md-table-cell>
        <md-table-cell>{{ size.recommended }}</md-table-cell>
...
export default {
  ...
  methods: {
    ...
    confirm() {
      axios
        .delete("admin/moving-size/" + this.selectedId)
        .then((res) => {
