    ...
    <md-table md-card>
      <md-table-toolbar>
        <div class="md-toolbar-section-start">
          <h1 class="md-title">Moving type</h1>
        </div>
      </md-table-toolbar>
      ...
      <md-table-row v-for="type in types" :key="type.id">
        <md-table-cell md-numeric>{{ type.id }}</md-table-cell>
        <md-table-cell>{{ type.title }}</md-table-cell>
        <md-table-cell>{{ type.code }}</md-table-cell>

        <md-table-cell md-label="Actions">
          <md-button class="md-icon-button md-primary" @click="edit(type)">
            <md-icon>edit</md-icon>
          </md-button>
          <md-button class="md-icon-button md-accent" @click="remove(type.id)">
...
export default {
  ...
  methods: {
    ...
    confirm() {
      axios
        .delete("admin/moving-types/" + this.selectedId)
        .then((res) => {
