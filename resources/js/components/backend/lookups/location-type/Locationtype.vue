<template>
  <div class="main-container">
    ...
    <md-dialog :md-active.sync="editTogal">
      ...
      <md-dialog-content>
        <Edit v-on:close-dialog="refresh" :location="location" />
      </md-dialog-content>
    </md-dialog>
    <!-- add dialog -->
    ...
    <md-table md-card>
      ...
      <md-table-row v-for="location in locations" :key="location.id">
        <md-table-cell md-numeric>{{ location.id }}</md-table-cell>
        <md-table-cell>{{ location.title }}</md-table-cell>
        <md-table-cell>{{ location.code }}</md-table-cell>

        <md-table-cell md-label="Actions">
          <md-button class="md-icon-button md-primary" @click="edit(location)">
            <md-icon>edit</md-icon>
          </md-button>
          <md-button class="md-icon-button md-accent" @click="remove(location.id)">
...
export default {
  ...
  methods: {
    ...
    confirm() {
      axios
        .delete("admin/location-type/" + this.selectedId)
        .then((res) => {
